...
# --- KONFIGURACE ---
st.set_page_config(layout="wide") # Širší layout pro lepší zobrazení sloupce
...
st.markdown("""
    ...
    /* Výchozí stav: Plovoucí tlačítko na desktopu skryjeme */
    [data-testid="stPopover"] { display: none; }

    /* Mobilní zobrazení: Šířka pod 800px */
...
def get_flag_html(country_name):
    iso_codes = {"Česko": "cz", "Slovensko": "sk", "Jižní Korea": "kr", "Německo": "de", "Francie": "fr", "Anglie": "gb", "Španělsko": "es", "Itálie": "it", "Brazílie": "br", "Argentina": "ar", "Mexiko": "mx", "Kanada": "ca", "Bosna a Hercegovina": "ba", "USA": "us", "Paraguay": "py", "Katar": "qa", "Švýcarsko": "ch", "Maroko": "ma", "Jihoafrická republika": "za", "Jar": "za"}
    iso = "xx"
            ...
            with c1:
                if is_closed: st.error("Tipování uzavřeno.")
                    ...
                    with st.form(f"f_{zapas['ID']}"):
                        ...
                        střelec = st.selectbox("Tip na střelce:", df_soupisky[df_soupisky['Tým'] == zapas['Domaci']]['Hráč'].tolist() + df_soupisky[df_soupisky['Tým'] == zapas['Hoste']]['Hráč'].tolist(), key=f"s_{zapas['ID']}")
