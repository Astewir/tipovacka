...
def get_base_body(row, z):
    ...
    elif ((tip_d > tip_h and skore_d > skore_h) or (tip_d < tip_h and skore_h > skore_d) or (tip_d == tip_h and skore_h == skore_h)):
        body += 1
...
with col_side:
    ...
    if st.session_state.faze == "Play-off":
        for name in df_po['Jméno']:
            body_p = get_body_pavouk(name, df_pavouk, df_realne)
            df_po.loc[df_po['Jméno'] == name, 'Body'] += body_p
    
    # Seřadíme a resetujeme index, aby i začínalo od 0
    lb = df_po.sort_values('Body', ascending=False).reset_index(drop=True)
    
    for i, row in lb.iterrows():
        # i nyní odpovídá pořadí (0, 1, 2, ...), proto 0=🥇, 1=🥈, 2=🥉
        medal = {0: "🥇", 1: "🥈", 2: "🥉"}.get(i, "👤")
