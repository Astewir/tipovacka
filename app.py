...
PARY = {
    "Terezka": "Vojtin", "Vojtin": "Terezka",
    "Mladej Tono": "Míša", "Míša": "Mladej Tono",
    "Lukáš": "Kačaba", "Kačaba": "Lukáš",
    "Starší Tono":"Džáma", "Džáma":"Starší Tono"  
}
...
def calc_body(row):
    ...
    # 1. Spočítáme základní body hráče
    body = get_base_body(row, z)
    
    # 2. Pokud je to párové kolo, najdeme partnera
    if int(row['ID_Zapasu']) == 25:
        partner = PARY.get(row['Jméno'])
        if partner:
            # Hledáme partnera v tabulce tipů
            tip_partnera = df_tipy[(df_tipy['ID_Zapasu'] == 25) & (df_tipy['Jméno'] == partner)]
            
            if not tip_partnera.empty:
                # DŮLEŽITÉ: Přičteme POUZE základní body partnera! 
                # Nikdy nepoužíváme partner_row['Body'], protože ty už obsahují bonusy.
                # Tímto se zbavíme zacyklení a závislosti na pořadí.
                body += get_base_body(tip_partnera.iloc[0], z)
...
with col_main:
    ...
    with tab1:
        for _, zapas in df_nedohrane.iterrows(): render_zapas(zapas)
    with tab2:
        for _, zapas in df_dohrane.iterrows(): render_zapas(zapas)
