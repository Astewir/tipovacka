...
def get_flag_html(country_name):
    # Mapování názvů zemí na ISO kódy (potřebné pro URL vlajek)
    iso_codes = {
        ...
        "Jar": "za" # Přidej i tuto variantu, pokud ji máš v tabulce
    }
    
    # Najde ISO kód (pokud existuje)
    iso = "xx"
    ...
    # Vrací HTML s obrázkem z webu (funguje všude a okamžitě)
...
def get_user_color(name):
    color_map = {"Vojtin": "#FF5733", "Terezka": "#33FF57"}
...
with col_main:
    ...
    for _, zapas in df_zapas.iterrows():
        ...
        <div style="background-color: #1e1e1e; padding: 15px; border-radius: 8px 8px 0 0; border-bottom: 4px solid {border_color}; display: flex; justify-content: space-between; align-items: center; color: white; margin-top: 15px;">
            <div style="width: 40%; text-align: left; font-weight:bold; font-size: 1.1em;">
                {get_flag_html(zapas['Domaci'])} {zapas['Domaci']}
            </div>
            <div style="width: 20%; text-align: center;">{middle_content}</div>
            <div style="width: 40%; text-align: right; font-weight:bold; font-size: 1.1em;">
                {zapas['Hoste']} {get_flag_html(zapas['Hoste'])}
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("Detaily a tipy"):
            ...
            with c2:
                ...
                for _, tip in df_tipy[df_tipy['ID_Zapasu'] == zapas['ID']].iterrows():
                    ...
                    st.markdown(f"""
                    <div style="background-color: #262730; padding: 10px 15px; margin-bottom: 8px; border-radius: 10px; border-left: 4px solid {user_color}; display: flex; justify-content: space-between; align-items: center; box-shadow: 2px 2px 5px rgba(0,0,0,0.3);">
